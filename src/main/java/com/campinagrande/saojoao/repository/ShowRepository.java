...
public interface ShowRepository extends JpaRepository<Show, Long> {
    ...
    @Query("SELECT s FROM Show s JOIN FETCH s.artista JOIN FETCH s.polo WHERE s.dataHora >= :start AND s.dataHora <= :end ORDER BY s.dataHora")
    List<Show> findByDataHoraBetween(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
}
